/* 
Программа создаёт три строки с помощью метода CreateString() (создает строку заданной длины 
из заданных символов): первая состоит только из цифр, вторая из букв русского алфавита, 
третья из букв латинского (все буквы строчные).
Далее написать метод MixUp() который в случайном порядке перемешивает две строки (параметра метода)
и меняет регистр (делает из строчной заглавную) с вероятностью 0,3.
Напишите метод, который из строки извлекает три изначальные строки.
Организуйте проверку написанных методов и отобразите ход работы программы в консоли.
*/
using System;
